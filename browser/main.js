...
color:randomByte() << 16 | randomByte() << 8 | randomByte()
});
}
...
const x2=w/2;
const y2=h/2;
geometry.vertices.push(new THREE.Vector3(x1,y1,z),//vertex0
...
const geometries={
box:new THREE.BoxBufferGeometry(boxSz,boxSz,boxSz),plane:planeGeometry(w*boxSz,h*boxSz),circle:new THREE.CircleGeometry(3.0,100)
};
const materials={
plane:new THREE.MeshBasicMaterial({color:0xa0a0a0}),circle:new THREE.MeshBasicMaterial({color:0xc0c0c0}),lost:new THREE.MeshBasicMaterial({color:0xcccccc})
};
...
const shadowZ=new THREE.Vector3(Math.cos(shadowYaw)*Math.cos(shadowPitch),Math.sin(shadowPitch),Math.sin(shadowYaw)*Math.cos(shadowPitch)).negate();
...
const camera=new THREE.OrthographicCamera(width/ -2,width/2,height/2,height/ -2,0.0,far);
camera.matrixAutoUpdate=false;
camera.matrix.makeBasis(shadowX,shadowY,shadowZ.negate()).setPosition(pos);
camera.updateMatrixWorld(true);
return {
size:new THREE.Vector2(width,height),far:far,pos:pos,xVec:shadowX,yVec:shadowY,zVec:shadowZ,camera:camera
};
}
...
"tDepth":{value:null},"size":{value:new THREE.Vector2(512,512)},"cameraNear":{value:1},"cameraFar":{value:100},"projectionXY":{value:new THREE.Vector2(1,1)}
},vertexShader:$('#ssao-vertex-shader').text(),fragmentShader:loadedShaders.ssao.fragment
},compose:{
uniforms:{
"size":{value:new THREE.Vector2(512,512)},"cameraMatrix":{value:new THREE.Matrix4()},"tDiffuse":{value:null},"tShadow":{value:null},"tAO":{value:null},"shadowFar":{value:shadow.far},"shadowX":{value:shadow.xVec},"shadowY":{value:shadow.yVec},"shadowZ":{value:shadow.zVec},"shadowOrigin":{value:shadow.pos},"shadowSize":{value:shadow.size}
},vertexShader:$('#compose-vertex-shader').text(),fragmentShader:loadedShaders.compose.fragment
},blurX:{
defines:{
"WHICHCOORD":"coord.x","DIMENSION":"size.x","SAMPLES":SSAO_BLUR_SAMPLES,"SIGMA":SSAO_BLUR_SIGMA
},uniforms:{
"tSource":{value:null},"size":{value:new THREE.Vector2(512,512)}
},vertexShader:$('#flat-vertex-shader').text(),fragmentShader:loadedShaders.blur.fragment
},blurY:{
defines:{
"WHICHCOORD":"coord.y","DIMENSION":"size.y","SAMPLES":SSAO_BLUR_SAMPLES,"SIGMA":SSAO_BLUR_SIGMA
},uniforms:{
"tSource":{value:null},"size":{value:new THREE.Vector2(512,512)}
},vertexShader:$('#flat-vertex-shader').text(),fragmentShader:loadedShaders.blur.fragment
},downsample:{
...
"tSource":{value:null},"size":{value:new THREE.Vector2(512,512)}
},vertexShader:$('#flat-vertex-shader').text(),fragmentShader:loadedShaders.downsample2d.fragment
}
};
...
const flatQuad=new THREE.Mesh(new THREE.PlaneBufferGeometry(2,2),shaders.blurX.material);
...
shaders.ssao.uniforms.projectionXY.value.set(camera.projectionMatrix.elements[0],camera.projectionMatrix.elements[5]);

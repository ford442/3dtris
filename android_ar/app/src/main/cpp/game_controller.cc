...
constexpr int64_t
MAX_FRAME_TIME=static_cast<int64_t>(0.1*1e9);
}
GameController::GameController() : game(buildGame(getRandomSeedFromTime())),state(State::WAITING_FOR_PLANE),prev_timestamp(0),changed_by_controls(false),active_anchor_index(-1){}
void GameController::onTrackingState(bool isTracking){
if(isTracking){
switch(state){
case State::WAITING_FOR_PLANE:state=State::WAITING_FOR_BOX;
LOGI("GameController: WAITING_FOR_BOX");
break;
case State::PAUSED_TRACKING_LOST:state=State::RUNNING;
...
switch(state){
case State::RUNNING:state=State::PAUSED_TRACKING_LOST;
LOGI("GameController: RUNNING -> PAUSED_TRACKING_LOST");
break;
case State::WAITING_FOR_BOX:state=State::WAITING_FOR_PLANE;
...
const glm::vec4 v=inv_view*glm::inverse(projection_mat)*glm::vec4(x_ndc,y_ndc,1.0,1.0);
...
auto that=this;
auto make_anchor=[anchor_origin,model_mat,view_mat,projection_mat,that](glm::vec3 r0,std::vector <Axis> rotation_arcs,float r){
RotationAnchor g;
...
const glm::vec3 z_anchor_r=glm::normalize(util::RotateOnly(glm::inverse(model_mat),-glm::vec3(dir.x,0,dir.z)));
...
rotate(anchor.arcs[best_arc_index].rotation_axis,ang > 0 ? RotationDirection::CCW :RotationDirection::CW);

...
constexpr size_t
kMaxAnchors=20;
template<class F> void getHit(ArSession *ar_session_,ArFrame *ar_frame_,float x,float y,bool polygonTest,F f){
...
if(AR_TRACKABLE_POINT == ar_trackable_type){
...
LOGE("MainApplication::onTap ArHitResult_acquireNewAnchor error");
return;
}
...
MainApplication::MainApplication(AAssetManager *asset_manager) : asset_manager_(asset_manager),game_controller_(),game_box_renderer_(game_controller_.getGame().getDimensions()),control_renderer_(game_controller_),
//debug_renderer_(),
game_model_mat_(1.0f),game_scale_(1.0f){
LOGD("OnCreate()");
}
...
case AR_INSTALL_STATUS_INSTALL_REQUESTED:install_requested_=true;
return;
}
...
void MainApplication::OnDisplayGeometryChanged(int display_rotation,int width,int height){
...
ArSession_setCameraTextureName(ar_session_,background_renderer_.GetTextureId());
...
background_renderer_.Draw(ar_session_,ar_frame_,game_controller_.getState() == GameController::State::RUNNING && !game_controller_.getGame().isOver());
...
ArLightEstimate_getState(ar_session_,ar_light_estimate,&ar_light_estimate_state);

// Set light intensity to default. Intensity value ranges from 0.0f to 1.0f.
float light_intensity=0.8f;
if(ar_light_estimate_state == AR_LIGHT_ESTIMATE_STATE_VALID){
ArLightEstimate_getPixelIntensity(ar_session_,ar_light_estimate,&light_intensity);
}
...
ArTrackable_getTrackingState(ar_session_,ar_trackable,&out_tracking_state);
...
ArTrackable_getTrackingState(ar_session_,ArAsTrackable(ar_plane),&plane_tracking_state);
...
getHit(ar_session_,ar_frame_,0.5f*width_,0.5f*height_,false,[session,&renderer,projection_mat,view_mat,&model_mat,&scale](ArAnchor *anchor){
...
if(game_controller_.getState() == GameController::State::WAITING_FOR_BOX){
getHit(ar_session_,ar_frame_, /*x, y,*/ 0.5f*width_,0.5f*height_,false,[&anchors,&controller](ArAnchor *anchor){

...
LineList lines;
auto makeArc=[&lines](glm::vec3 pos,glm::vec3 r,glm::vec3 dir){
...
if(i > 0){
lines.push_back(std::make_pair(pos+pPrev,pos+p));
}
pPrev=p;
}
};
...
lines.push_back(std::make_pair(drop_arrow.r-drop_d,down));
lines.push_back(std::make_pair(down,down+(drop_arrow.side_dir-drop_arrow.dir)*head_scale));
lines.push_back(std::make_pair(down,down+(-drop_arrow.side_dir-drop_arrow.dir)*head_scale));
return lines;
}
LineList drawMoveArrows(const GameController &controller){
LineList lines;
auto makeArrow=[&lines](glm::vec3 pos0,glm::vec3 dir,glm::vec3 side){
...
ControlRenderer::ControlRenderer(const GameController &controller) : controller_(controller),arrows_(drawMoveArrows(controller_)){}
...
void ControlRenderer::Draw(const glm::mat4 &projection_mat,const glm::mat4 &view_mat,const glm::mat4 &model_mat,float game_scale){
if(!shader_program_){

...
Axis exAx0,exAx1;
switch(rot.axis){
case Axis::X:exAx0=Axis::Y;
exAx1=Axis::Z;
break;
case Axis::Y:exAx0=Axis::Z;
exAx1=Axis::X;
break;
case Axis::Z:exAx0=Axis::X;
exAx1=Axis::Y;
break;
}
...
return Block{
pos_methods::sum(target.pos,delta),target.pieceId
};
}
Block rotate(const Block &target,Rotation rot){
return Block{
pos_methods::rotate(target.pos,rot),target.pieceId
};
}
}
// snippet from http://blog.madhukaraphatak.com/functional-programming-in-c++/
template<typename Collection,typename unop> Collection map(Collection col,unop op){
std::transform(col.begin(),col.end(),col.begin(),op);
return col;
}
Piece Piece::translated(Pos3d delta) const{
return Piece{
pos_methods::sum(center,delta),blocks
};
}
Piece Piece::rotated(Rotation rot) const{
return Piece{
center,map(blocks,[rot](Block b){return block_methods::rotate(b,rot);})
};
}
...
return direction > 0 ? *std::max_element(coords.begin(),coords.end()) :*std::min_element(coords.begin(),coords.end());
}
...
if(diff > 0){
return translated(pos_methods::setElement(Pos3d{0,0,0},axis,diff*direction));
}
return *this;
}

...
ConcreteGame::ConcreteGame(unsigned int randomSeed) : gameBox(game_config::DIMENSIONS),blockArray(gameBox),pieceGenerator(gameBox,randomSeed),activePiece(pieceGenerator.nextPiece()),score(0),alive(true),timeToNextDownMs(game_config::DROP_INTERVAL_MS),nDroppedPieces(0){}
...
return setActivePieceIfFits(gameBox.translateToBounds(activePiece.rotated(rot)));
}
